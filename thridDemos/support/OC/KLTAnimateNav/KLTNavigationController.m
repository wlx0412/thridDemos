...
- (UIViewController *)popViewControllerAnimated:(BOOL)animated
{
    ...
    BOOL canGoBack = [self.visibleViewController performSelector:@selector(goBack)];
    if (canGoBack) {
...
- (void)panGestureRec:(UIScreenEdgePanGestureRecognizer *)panGestureRec
{
    ...
    if(visibleVC == self.viewControllers[0]) return;
    //手势判断之前调用goBack方法,确认是否能返回
    if ([visibleVC respondsToSelector:@selector(goBack)] && ![(FR_BaseViewController *)visibleVC goBack]) { return; }
...
- (void)dragEnd
{
    ...
    if (translateX <= 40) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            [self->_screenshotImgs removeLastObject];

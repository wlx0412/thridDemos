...
open class JXPopupViewUpwardAnimator: JXPopupViewBaseAnimator {
    open override func setup(contentView: UIView, backgroundView: JXBackgroundView, containerView: UIView) {
        ...
        //初始化后可能调整过contentView的frame,导致targetRect不准确,故调整为block内用时获取
        dismissAnimateBlock = {
            var frame = contentView.frame
            frame.origin.y = containerView.bounds.size.height
...
open class JXPopupViewSprintZoomInOutAnimator: JXPopupViewBaseAnimator {
    open override func setup(contentView: UIView, backgroundView: JXBackgroundView, containerView: UIView) {
        contentView.alpha = 0
        backgroundView.alpha = 0
    ...
    open override func display(contentView: UIView, backgroundView: JXBackgroundView, animated: Bool, completion: @escaping () -> ()) {
        if animated {
            UIView.animate(withDuration: displayDuration, delay: 0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0.5, options: displayAnimationOptions, animations: {
                self.displayAnimateBlock?()
            }) { (finished) in
                completion()
            }
...
// MARK: - 自定义弹框
extension JXPopupView {
    static func showContent(_ contentView: UIView,_ containerView: UIView? = UIApplication.mainWindow(),_ touchOutsideCancel: Bool? = true,_ positionType: ContentPositionType? = .center) -> JXPopupView{
            var popupView:JXPopupView!
            
            if positionType == .center {
                popupView = JXPopupView(containerView: containerView!, contentView: contentView, animator: JXPopupViewZoomInOutAnimator())
                contentView.snp.makeConstraints { (make) in
                    make.center.equalToSuperview()
                    make.width.equalTo(Size.kAdaptWidth(300))
                }
                contentView.setBorder(radius: Size.kAdaptWidth(4))
            }else if positionType == .bottom{
                popupView = JXPopupView(containerView: containerView!, contentView: contentView, animator: JXPopupViewUpwardAnimator())

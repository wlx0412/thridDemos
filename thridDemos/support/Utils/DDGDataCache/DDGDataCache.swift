...
extension DDGDataCache
{
    ...
    public class func cacheJsonWithURL(_ URL: String,subPath:String = "") -> AnyObject? {
        let path: String = self.cacheFilePathWithURL(URL, path: DDGCacheKeyPath,subPath: subPath)
    ...
    public class func cachePath() -> String {
        let pathOfLibrary = NSSearchPathForDirectoriesInDomains(FileManager.SearchPathDirectory.documentDirectory, FileManager.SearchPathDomainMask.userDomainMask, true)[0] as NSString
        let path = pathOfLibrary.appendingPathComponent(DDGCacheKeyPath)
        return path
    }
    /**
     获取子缓存路径
    ...
    public class func clearAllCache() -> Bool{
        ...
        {
            Util.printX("clearCache failed , error = \(error)")
            return false
        }
    }
    /**

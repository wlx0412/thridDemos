...
class EWDatePickerViewController: UIViewController {
    ...
    var years: [Int]? //年份数组,默认当前年的前后各10年
    ...
    override func viewDidLoad() {
        ...
        years = [Int]((currentDateCom.year! - 10)...(currentDateCom.year! + 10))
        drawMyView()
    }
    // MARK: - Func
    ...
    @objc func onClickSure() {
        ...
        if components == 3 {
            dateString = String(format: "%02ld-%02ld-%02ld", year ?? currentDateCom.year!, month ?? currentDateCom.month!, day ?? currentDateCom.day!)
        } else {
            dateString = String(format: "%02ld-%02ld", year ?? currentDateCom.year!, month ?? currentDateCom.month!)
        }
...
extension EWDatePickerViewController:UIPickerViewDelegate,UIPickerViewDataSource {
    ...
    func pickerView(_ pickerView: UIPickerView, widthForComponent component: Int) -> CGFloat {
        return ScreenInfo.Width / CGFloat(components)
    }
    ...
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        if component == 0 {
            return "\(years?[row] ?? currentDateCom.year!)\("年")"
        } else if component == 1 {
            return "\(row + 1)\("月")"
        } else {
            return "\(row + 1)\("日")"

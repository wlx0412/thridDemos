...
class EWPopMenuView: UIView {
    ...
    init(width: CGFloat, height: CGFloat, point: CGPoint, items: [String], imgSource: [String],  action: ((Int) -> Void)?) {
        super.init(frame:CGRect(x: 0, y: 0, width: EScreenInfo.Width, height: EScreenInfo.Height))
        drawMyTableView()
        /// view全屏展示
        self.frame = CGRect(x: 0, y: 0, width: EScreenInfo.Width, height: EScreenInfo.Height)
        /// 获取起始点
        self.point = CGPoint(x: point.x, y: EScreenInfo.navigationHeight + point.y)
...
extension EWPopMenuView:UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let show = showNumberLab[indexPath.row, true] {
            cell.numberLab.isHidden = !show
        } else {

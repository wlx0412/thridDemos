...
class MNMarqueeView: UIView {
    ...
    /// 富文本列表
    var attributeTextList = [AttributedString]() {
        willSet {
            stop()
        }
        didSet {
...
fileprivate extension MNMarqueeView {
    ...
    func increaseIndex() {
        if attributeTextList.count > 0 {
            nextIndex = (nextIndex + 1) % attributeTextList.count
            return
        }
    ...
    func resetIndex() {
        if attributeTextList.count > 0 {
            nextIndex = attributeTextList.isEmpty ? NSNotFound : 0
            return
        }
...
fileprivate extension MNMarqueeView {
    ...
    func addOnScreenTextLabel() {
        ...
        if attributeTextList.count > 0{
            textLabel.attributed.text = attributeTextList[currentIndex]
        }else{

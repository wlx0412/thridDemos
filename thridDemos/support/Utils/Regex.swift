...
public class Regex {
    ...
    static let RegexPatternRegex = try? Regex(RegexEscapePattern)
    ...
    public func matches(testStr: String) -> [NSTextCheckingResult] {
        let matches = self.expression.matches(in: testStr, options: [], range: NSRange(location: 0, length: testStr.count))
        return matches
    }

    public func rangeOfFirstMatch(testStr: String) -> NSRange {
        return self.expression.rangeOfFirstMatch(in: testStr, options: [], range: NSRange(location: 0, length: testStr.count))
    }
